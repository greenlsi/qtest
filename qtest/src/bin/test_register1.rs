...
async fn main() {
    ...
    let pin_leido2 = gpioc_idr.read_pin(13, &mut parser).await;
    let registro_leido2 = gpioc_idr.read_register(&mut parser).await;
    println!("{registro_leido2}, {pin_leido2}");
    tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
    ...
    let pin_leido3 = gpioc_idr.read_pin(13, &mut parser).await;
    let registro_leido3 = gpioc_idr.read_register(&mut parser).await;
    println!("{registro_leido3}, {pin_leido3}");
    ...
    let pin_leido4 = gpioc_idr.read_pin(13, &mut parser).await;
    let registro_leido4 = gpioc_idr.read_register(&mut parser).await;
    println!("{registro_leido4}, {pin_leido4}");
    tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
}
